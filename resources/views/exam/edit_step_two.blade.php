...
@section('content')
	<section class="content">
		...
		<form method='get' action='{!!url("portal/editexam-stepone/10/edit")!!}'>
			<button class='btn btn-danger'>Go Back</button>
		</form>
		<br>
		<form onsubmit="return validate_question()" method='POST' action='{!!url("portal/update-exam")!!}' class="form-inline">
			...
			<table class="table table-striped" id="example">
				...
				@foreach($questions as $question)
					<tr>
						<td><input  class="question_check" type="checkbox" data-ques_mark="{{$question->mark}}" name="selected_questions[]"  <?php if(in_array($question->questionID,$old_questions)){echo "checked";}?>  value="<?php echo $question->questionID; ?>"></td>
                        <td><?php
